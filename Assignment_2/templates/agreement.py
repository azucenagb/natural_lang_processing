...
def check_node(tr):
    ...
    if (rule == 'S -> WHICH Nom QP QM'):
        ...
        boolean1 = (matches(V_phrase_num(tr[0]), N_phrase_num(tr[1])))
        boolean2 = (V_phrase_num(tr[2]) == 'p')
...
# Example:
    ...
    #print tag_word(lx, 'purple')
    #print tag_word(lx, 'ducks')
    #print tag_word(lx, 'fly')
    #print tag_word(lx, '?')

    #all_parses(['Who','likes','John','?'], lx)[0].draw()
    #tr0 = all_valid_parses(lx, ['Who','likes','John','?'])[0]
    #tr = restore_words(tr0,['Who','likes','John','?'])
    #tr.draw()
    #all_parses(['Which','purple','ducks','fly', '?'], lx)[0].draw()
    #tr0 = all_valid_parses(lx, ['Which','purple','ducks','fly', '?'])[0]
    #tr = restore_words(tr0,['Which','purple','ducks','fly', '?'])
    #tr.draw()
