...
class MH_Typechecker {
	...
	throws TypeError, UnknownVariable {
        ...
        if (exp.isVAR()) { return (MH_TYPE) env.typeOf(exp.value());}
        if (exp.isINFIX()) {
        	if (!((computeType(exp.first(), env)).equals(IntegerType))){
        		throw new TypeError ("The types of the elements in an infix operation must be integers");
        	}
        	if (!((computeType(exp.second(), env)).equals(IntegerType))) {
        		throw new TypeError("The types of the elements in an infix operation must be integers");
        	}
        	if (exp.infixOp().equals("==")) {
        		return BoolType;
        	}
        	if (exp.infixOp().equals("<=")) {
        		return BoolType;
        ...
        if (exp.isIF()) {
        	if (!((computeType(exp.first(), env)).equals(BoolType))) {
        		throw new TypeError("The condition in an if statement must be a boolean");
        	}
        	if (!(computeType(exp.third(), env)).equals(computeType(exp.second(), env))) {
        		throw new TypeError("The two possible values for an if statement must be of the same type");
        	}
        	return computeType(exp.second(), env);
        }
        
        if (exp.isAPP()) {
        	MH_TYPE func = computeType(exp.first(), env);
        	MH_TYPE arguments = computeType(exp.second(), env);
        	if (!func.isFun()) {
        		throw new TypeError("The first element in an application must be a function");
        	}
        	if (func.left().equals(arguments)) {
        		return func.right();
        	} else throw new TypeError("The element types must match the argument types of the function"); 
        }
        throw new TypeError("The expression is not of an accepted type");
